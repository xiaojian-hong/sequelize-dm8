...
class DmdbQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    return `CREATE TABLE IF NOT EXISTS ${table} (${attributesClause}) ${comment}${charset}${collation}${initialAutoIncrement}${rowFormat};`;
  }
  ...
  showIndexesQuery(tableName, options) {
    let sql = `SELECT TABLE_NAME,INDEX_NAME FROM DBA_IND_COLUMNS WHERE TABLE_NAME= '${tableName}'`;
    if (options && options.database) {
      sql += ` FROM '${options.database}'`;
    }
    return sql;
  }
